...
class SlotModel(nn.Module):
    def __init__(self, args,class_means):
        ...
        if self.use_slot:
            ...
            self.conv1x1 = nn.Conv2d(self.channel, dim, kernel_size=1, stride=1)
            if args.pre_trained:
                self.dfs_freeze(self.backbone, args.freeze_layers)
            self.slot = SlotAttention(num_classes=args.num_classes, dim=dim, class_means=class_means, vis=args.vis,vis_id=args.vis_id, loss_status=args.loss_status, power=args.power)
            self.position_emb = build_position_encoding('sine', hidden_dim=dim)
