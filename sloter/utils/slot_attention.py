...
class SlotAttention(nn.Module):
    ...
    def forward(self, inputs, inputs_x):
        ...
        # Ensure the ones vector has the same size as the last dimension of attn
        ones_vec = torch.ones(attn.size(-1), device=attn.device)
        # attn_aggregated = torch.matmul(attn, ones_vec).squeeze(2)  # Resulting shape: [b, n]
        attn_aggregated = torch.matmul(attn, ones_vec)  
